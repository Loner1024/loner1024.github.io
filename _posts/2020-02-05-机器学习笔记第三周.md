...
所以，当 $\theta^Tx \>=0$ 时，$h\_\theta(x) \>= 0.5$,

当 $\theta^Tx \<0$ 时，$h\_\theta(x) \< 0.5$
...
$J(\theta)=\frac{1}{m}Cost(h\_\theta(x^{(i)}),y^{(i)})$

$=-\frac{1}{m}\sum\limits\_{i=1}^{m}{[{{y}^{(i)}}\log \left( {h_\theta}\left( {{x}^{(i)}} \right) \right)+\left( 1-{{y}^{(i)}} \right)\log \left( 1-{h_\theta}\left( {{x}^{(i)}} \right) \right)]}$
...
​                                                   ${\theta\_0}:={\theta\_0}-a\frac{1}{m}\sum\limits_{i=1}^{m}{(({h_\theta}({{x}^{(i)}})-{{y}^{(i)}})x\_{0}^{(i)}})$ 

​                                                   ${\theta\_j}:={\theta\_j}-a[\frac{1}{m}\sum\limits_{i=1}^{m}{(({h_\theta}({{x}^{(i)}})-{{y}^{(i)}})x\_{j}^{\left( i \right)}}+\frac{\lambda }{m}{\theta\_j}]$ 

​                                                             $for$ $j=1,2,...n$

​                                                   }

其中${\theta\_j}$可以简化为：${\theta\_j}:={\theta\_j}(1-a\frac{\lambda }{m})-a\frac{1}{m}\sum\limits_{i=1}^{m}{({h_\theta}({{x}^{(i)}})-{{y}^{(i)}})x\_{j}^{\left( i \right)}}$ 
...
Logistic 回归的正则化和线性回归相似，但$h\_\theta(x)=\frac{1}{{1+e^{-\theta^Tx}}}$
